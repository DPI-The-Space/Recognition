...
A team engagement website built for the DPI org to recognize each other, share life updates, browse team profiles, and stay connected.

## Features

- **Work Announcements** — Post and browse peer recognition with categories, likes, and comments
- **Life Announcements** — Share personal milestones (birthdays, weddings, new pets, etc.)
- **About The Team** — Profile cards for every team member with details like hobbies, fun facts, and drink orders
- **Directory** — Org chart view with all teams, managers, and job titles in a 4-column layout
- **Calendar** — Team events, auto-populated birthdays, and PTO tracking
- **Team Resources** — Quick links to Spotify playlist, AI Trello board, health resources, Charter newsletter, and Panorama
- **Profiles** — Editable profiles with Charter job title, "What I Do" freeform field, and personal details
- **Notifications** — Pulsing red dots on tabs when new content is posted
- **Login** — Team-based login with 24-hour session persistence

## Files

| File | Purpose |
|------|---------|
| `index.html` | Main application — all tabs, UI, styles, and JavaScript logic |
| `profile.html` | Profile editing page — personal details, avatar upload, birthday |
| `TheSpace-Logo-Blue.png` | Blue logo displayed in the header banner |
| `TheSpace-Logo-White.png` | White logo used as browser tab favicon and on the login page |
| `HOW-TO-UPDATE.md` | Guide for updating team rosters, job titles, and resources |

## Tech Stack

- **Frontend:** Single-page HTML/CSS/JS (no build tools or frameworks)
- **Database:** Firebase Firestore (profiles, posts, comments, calendar events)
- **APIs:** Giphy (GIF search in posts)
- **Hosting:** Static file — runs from any web server or locally via `file://`

## Cost

$0/month — all usage fits within Firebase's free tier for a team of this size.
...
Reach out to Malia Jones (malia.jones@spectrum.com) for questions or suggestions.
